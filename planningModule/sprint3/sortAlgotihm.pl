...
% dynamic fact to store the better solution in a certain moment
...
round_robin_schedule_surgeries(Day) :-
    ...
    % Start round-robin distribution
...
schedule_single_surgery(OpCode, Room, Day) :-
    ...
    % Remove intervals too short for surgery
    remove_unf_intervals(TSurgery, LIntAgDoctorsRoom, LPossibilities),
    ...
    % Update doctorsagendas
    insert_agenda_doctors((TinS, TfinS, OpCode), Day, LDoctors).
 
% Calculate the total time occupied by all surgeries for a specific day
calculate_total_time(Day, TotalTime) :-
    findall(Room, agenda_operation_room(Room, Day, _), LRooms),
    calculate_rooms_total_time(LRooms, Day, TotalTime).
 
calculate_rooms_total_time([], _, 0).
calculate_rooms_total_time([Room|LRooms], Day, TotalTime) :-
    agenda_operation_room1(Room, Day, Schedule),
    calculate_schedule_time(Schedule, RoomTime),
    calculate_rooms_total_time(LRooms, Day, RestTime),
    TotalTime is RoomTime + RestTime.
 
calculate_schedule_time([], 0).
calculate_schedule_time([(Start, End, _)|Rest], TotalTime) :-
    Duration is End - Start + 1,
    calculate_schedule_time(Rest, RestTime),
    TotalTime is Duration + RestTime.
 
% Existing print schedule predicates remain the same
print_final_schedule_with_total_time(Day) :-
    nl,
    write('      -  Surgery Schedule for Day: '), write(Day), write(' '), nl,
    nl,
    findall(Room, agenda_operation_room(Room, Day, _), LRooms),
    print_rooms_schedule(LRooms, Day),
    calculate_total_time(Day, TotalTime),
    nl,
    write('     - Total Surgery Time Occupied: '), write(TotalTime), write(' minutes ***'), nl.
...
print_rooms_schedule([Room|LRooms], Day) :-
    ...
    write('       *** Room '), write(Room), write(' Schedule: ***'), nl,
...
print_schedule([(Start, End, OpCode)|Rest]) :-
    write('  Surgery Code - '), write(OpCode), 
    write(' TimeTable :  '), write(Start), write(' to '), write(End), nl,
    print_schedule(Rest).
