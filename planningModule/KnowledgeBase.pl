...
:- discontiguous obtain_better_sol/5.
:- discontiguous obtain_better_sol1/2.
...
staff(d001,doctor,orthopaedist,[so2,so3]).
timetable(d001,20241028,(480,1200)).
agenda_staff(d001,20241028,[(720,790,m01),(1080,1140,c01)]).
...
% Define o update_better_sol/4 para atualizar a solução se o novo TFinOp for menor que o atual

update_better_sol(Day, Room, AgOpRoom, LOpCode) :-
    evaluate_final_time(AgOpRoom, LOpCode, TFinOp),
    better_sol(Day, Room, _, _, CurrentTFinOp),
    TFinOp < CurrentTFinOp, % Atualiza se o tempo final da nova solução for menor
    retract(better_sol(Day, Room, _, _, _)),
    assert(better_sol(Day, Room, AgOpRoom, LOpCode, TFinOp)).
